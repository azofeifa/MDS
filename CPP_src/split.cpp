...
vector<string> split_by_ws(string line, string delim){
	vector<string> tokens;
	istringstream iss(line);
	string token;
	while(std::getline(iss, token , ' ' )){   // but we can specify a different one
...
vector<string> split_by_us(string line, string delim){
	vector<string> tokens;
	istringstream iss(line);
	string token;
	while(std::getline(iss, token , '_' )){   // but we can specify a different one
		tokens.push_back(token);
	}
	return tokens;
}
